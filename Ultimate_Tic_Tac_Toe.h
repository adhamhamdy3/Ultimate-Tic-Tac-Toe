...
template<typename T>
int Local_Board<T>::get_columns()const { return columns; }

template<typename T>
int Local_Board<T>::get_rows()const { return rows; }

template<typename T>
T Local_Board<T>::get_board_value(int x, int y)const {
...
void Ultimate_Board<T>::display_board() {
    int localRows = this->boards[0][0]->get_rows();
    int localCols = this->boards[0][0]->get_columns();
        ...
        for (int localRow = 0; localRow < localRows; ++localRow) {
            for (int j = 0; j < this->columns; ++j) {
                for (int localCol = 0; localCol < localCols; ++localCol) {
                    ...
                    cout << this->boards[i][j]->get_board_value(localRow, localCol) << " ";
                }
                cout << "   || ";
            }
            cout << endl;
        }
        // Print row separator for local boards
        for (int j = 0; j < this->columns; ++j) {
            cout << "-----------------------------   ";
        }
        cout << endl;
